...
docker run -p $NEWPORT:8077 -p $NEWPORTplus1:8078 --mount type=bind,source=$DATADIR,target=/readonlydir,readonly --mount type=bind,source=${PWD}/docker_results,target=/iodir -it --pull always franchesoni/binann:0.1 $NEWIPADDRESS $NEWPORT